...
| 3 | Dashboard | `dashboard.md` | Live experiment dashboard — Gantt, architecture flow, cell-completion grid, C1 invariants | Stage 0 | Weekly (manual until weekly_dashboard.yml CI lands) | 2026-05-19 | ✅ Current (M3 exit; 16 deviation entries; G-pipe + L-pipe + OSF freeze rows added) |
...
| 5 | Deviation Log | `deviation_log.md` | Human-readable mirror of `deviation_log.jsonl` — HMAC-signed deviation entries | Stage 0 | After every `bin/log_deviation.py` append | 2026-05-19 | ✅ Current (16 entries; M3 adds entries 11–16: schema bump, re-capture, orchestration, threshold, model, runtime) |
...
| 16 | Tracking Documents Register | `tracking_documents_register.md` | This document — master index of all tracking docs | Stage 0 | On any tracking-doc add/remove/rename | 2026-05-19 | ✅ Current (M3 rows updated for 6 documents) |
...
## Milestone 3 OSF Artefacts

The following artefacts are generated by `bin/verify_osf_freeze.py --generate` and verified by the `osf-freeze-verify` CI job on every push. They are not tracking documents in the schema-enforced sense but are registered here as research artefacts per G3-8.

| Path | Type | Purpose | Generation | Active since |
|---|---|---|---|---|
